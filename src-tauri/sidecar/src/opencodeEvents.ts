...
export function toCodeMuxEvent(event: unknown, context: OpenCodeEventContext): CodeMuxEvent[] {
  ...
  if (context.seenEventIds?.has(identity)) {
    return [];
  }
  ...
  const properties = asRecord(record?.properties) ?? {};
  const sessionId = readString(properties.sessionID) ?? context.agentSessionId;
  ...
  switch (type) {
    case 'message.part.updated': {
      ...
      if (partType === 'text' || partType === 'reasoning') {
        ...
        if (status === 'pending' || status === 'running') {
          ...
          const output = status === 'completed' ? readString(state?.output) ?? '' : readString(state?.error) ?? 'OpenCode tool failed';
          events.push({
    ...
    case 'message.updated': {
      ...
      if (error) {
        events.push(...buildFailureEvents(context, error));
      }
      break;
    }
    case 'session.status': {
    ...
    case 'session.idle':
      events.push(...buildResultEvents(context, 'success'));
      break;
    case 'session.error': {
      ...
      events.push(...buildResultEvents(context, status));
      break;
    }
    ...
    case 'connection.error': {
      ...
      events.push(...buildResultEvents(context, 'error'));
      break;
    }
    ...
    case 'session.aborted':
      ...
      events.push(...buildResultEvents(context, 'interrupted'));
      break;
    default:
...
function buildResultEvents(context: OpenCodeEventContext, status: OpenCodeResultStatus): CodeMuxEvent[] {
  return [{ ...buildOpenCodeResultEvent({ context, usage: context.usage ?? emptyUsage(), durationMs: context.durationMs ?? 0, status }) as CodeMuxEvent, ...routingMetadata(context, context.agentSessionId) }];
}

function buildFailureEvents(context: OpenCodeEventContext, error: unknown): CodeMuxEvent[] {
  const status = isInterruptedError(error) ? 'interrupted' : 'error';
  return [
    buildEnvelope({ type: 'error', subtype: status, error: errorMessage(error) }, context, context.agentSessionId),
    ...buildResultEvents(context, status),
  ];
}
...
function buildAssistantEnvelope(context: OpenCodeEventContext, sessionId: string | undefined, content: Array<Record<string, unknown>>): CodeMuxEvent {
  return {
    ...buildAssistantEvent({ sessionId: context.sessionId, content: content as AssistantContentBlock[] }),
    ...routingMetadata(context, sessionId),
  };
}
...
function stableStringify(value: unknown): string {
  ...
  if (record) return `{${Object.keys(record).sort().map((key) => `${JSON.stringify(key)}:${stableStringify(record[key])}`).join(',')}}`;
