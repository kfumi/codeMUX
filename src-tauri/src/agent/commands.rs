...
fn find_claude_session_dir(claude_dir: &Path, claude_session_id: &str) -> Option<PathBuf> {
    let jsonl_path = find_claude_session_jsonl(claude_dir, claude_session_id)?;
    let project_dir = jsonl_path.parent()?;
    Some(project_dir.join(claude_session_id))
}

fn extract_agent_id_from_meta_filename(path: &Path) -> Option<String> {
...
fn extract_agent_id_from_jsonl_filename(path: &Path) -> Option<String> {
    let file_name = path.file_name()?.to_str()?;
...
fn collect_subagent_index_from_dir(subagents_dir: &Path) -> HashMap<String, String> {
    use std::fs;
    ...
    for entry in entries.flatten() {
        ...
        let Some(agent_id) = extract_agent_id_from_meta_filename(&path) else {
            continue;
        };
        ...
        index.insert(tool_use_id.to_string(), agent_id);
    }

    index
}

fn collect_subagent_jsonl_agent_ids(subagents_dir: &Path) -> Vec<String> {
    use std::fs;
    ...
    entries
        ...
        .filter_map(|entry| extract_agent_id_from_jsonl_filename(&entry.path()))
        .collect()
}

fn collect_agent_tool_use_ids_from_main_jsonl(jsonl_path: &Path) -> Vec<String> {
    use std::fs::File;
    ...
    for line_result in reader.lines() {
        ...
        for block in content {
            let tool_name = block.get("name").and_then(|entry| entry.as_str());
            if !matches!(tool_name, Some("Agent" | "Task" | "subagent")) {
                continue;
            }
            ...
            if let Some(tool_use_id) = block
                ...
                .filter(|entry| !entry.is_empty())
            {
                tool_use_ids.push(tool_use_id.to_string());
            }
        }
    }
...
fn find_subagent_session_jsonl(
    ...
    // Strategy 1: Find the main session JSONL, then look for subagents/ in the session dir.
    // Path: {project_dir}/{claude_session_id}/subagents/agent-{agent_id}.jsonl
    if let Some(session_dir) = find_claude_session_dir(claude_dir, claude_session_id) {
        let subagent_jsonl = session_dir
            .join("subagents")
            .join(format!("agent-{}.jsonl", agent_id));
        if subagent_jsonl.exists() {
            return Some(subagent_jsonl);
        }
        return None;
    }

    // Strategy 2: Fallback — scan all project dirs and their session subdirs.
    for entry in fs::read_dir(&projects_dir).ok()?.flatten() {
        if !entry.file_type().map(|t| t.is_dir()).unwrap_or(false) {
            continue;
        }
        let session_dir = entry.path().join(claude_session_id);
        let subagent_jsonl = session_dir
            .join("subagents")
            .join(format!("agent-{}.jsonl", agent_id));
        if subagent_jsonl.exists() {
...
#[tauri::command]
pub async fn load_claude_subagent_index(
    state: State<'_, crate::AppState>,
    app_session_id: String,
) -> Result<HashMap<String, String>, String> {
    debug!(target: "agent", "Loading Claude sub-agent index for app_session_id={}", app_session_id);
    ...
    let claude_dir = home_dir()?.join(".claude");
    let Some(session_jsonl) = find_claude_session_jsonl(&claude_dir, &claude_session_id) else {
        info!(
            target: "agent",
            "Claude JSONL not found while loading sub-agent index for app_session_id={} claude_session_id={}",
            app_session_id,
            claude_session_id
        );
        return Ok(HashMap::new());
    ...
    let Some(project_dir) = session_jsonl.parent() else {
        return Ok(HashMap::new());
    };
    let subagents_dir = project_dir.join(&claude_session_id).join("subagents");
    let mut index = collect_subagent_index_from_dir(&subagents_dir);

    if index.is_empty() {
        let tool_use_ids = collect_agent_tool_use_ids_from_main_jsonl(&session_jsonl);
        let agent_ids = collect_subagent_jsonl_agent_ids(&subagents_dir);
        if tool_use_ids.len() == 1 && agent_ids.len() == 1 {
            index.insert(tool_use_ids[0].clone(), agent_ids[0].clone());
        }
    }

    info!(
        target: "agent",
        "Loaded {} Claude sub-agent index entries for app_session_id={}",
        index.len(),
        app_session_id
    );
    Ok(index)
...
#[tauri::command]
pub async fn load_subagent_session_events(
    state: State<'_, crate::AppState>,
    app_session_id: String,
    agent_id: String,
) -> Result<Vec<serde_json::Value>, String> {
    ...
    debug!(target: "agent", "Loading sub-agent session events for app_session_id={} agent_id={}", app_session_id, agent_id);
    ...
    let claude_dir = home_dir()?.join(".claude");
    let Some(jsonl_path) = find_subagent_session_jsonl(&claude_dir, &claude_session_id, &agent_id)
    else {
        info!(
            target: "agent",
            "Sub-agent JSONL not found for app_session_id={} agent_id={}",
            app_session_id,
            agent_id
        );
        return Ok(messages);
    ...
    debug!(target: "agent", "Reading sub-agent JSONL from {}", jsonl_path.display());

    let file = fs::File::open(&jsonl_path)
        .map_err(|e| format!("Failed to open sub-agent JSONL: {}", e))?;
    let reader = BufReader::new(file);
    ...
    info!(target: "agent", "Loaded {} sub-agent messages for app_session_id={} agent_id={}", messages.len(), app_session_id, agent_id);
    Ok(messages)
}
...
mod tests {
    ...
    #[test]
    fn collect_subagent_index_maps_meta_tool_use_id_to_agent_id() {
        use std::fs;

        let base =
            std::env::temp_dir().join(format!("codemux-subagent-index-{}", uuid::Uuid::new_v4()));
        let subagents_dir = base.join("subagents");
        fs::create_dir_all(&subagents_dir).unwrap();
        fs::write(
            subagents_dir.join("agent-a6cae6d569918e2d3.meta.json"),
            r#"{"agentType":"Explore","description":"Read package.json","toolUseId":"call_22bf1cc2e7484a108461feb0"}"#,
        )
        .unwrap();

        let index = collect_subagent_index_from_dir(&subagents_dir);

        assert_eq!(
            index
                .get("call_22bf1cc2e7484a108461feb0")
                .map(String::as_str),
            Some("a6cae6d569918e2d3")
        );
    ...
    #[test]
    fn collect_subagent_index_ignores_malformed_meta_files() {
        use std::fs;
        ...
        let subagents_dir = base.join("subagents");
        fs::create_dir_all(&subagents_dir).unwrap();
        fs::write(subagents_dir.join("agent-bad.meta.json"), "{not json").unwrap();
        fs::write(
            subagents_dir.join("agent-good.meta.json"),
            r#"{"toolUseId":"call_good"}"#,
        )
        .unwrap();

        let index = collect_subagent_index_from_dir(&subagents_dir);

        assert_eq!(index.get("call_good").map(String::as_str), Some("good"));
